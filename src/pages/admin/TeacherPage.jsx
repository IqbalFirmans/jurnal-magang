...
import RollingStudentsModal from '../../components/teachers/RollingStudentModal';
...
  const [selectedTeacherForRolling, setSelectedTeacherForRolling] = useState(null);
...
    setFormApiErrors(null);
    ...
    setSelectedTeacherForRolling(null);
  }, []);
...
      }));
      setTeachers(mapped);
      console.log('Mapped teacher: ', mapped)
    } catch (err) {
...
  const handleOpenRollingModal = useCallback((teacher) => {
    setSelectedTeacherForRolling(teacher);
    setIsRollingModalOpen(true);
  }, []);

const handleSaveRolling = useCallback(async (teacherId, studentIds) => {
        try {
            await teacherApi.rollStudents(teacherId, studentIds); 
            showToast('Siswa berhasil di-rolling!', 'success');
        } catch (err) {
            console.error('Gagal rolling siswa:', err.response?.data || err.message);
            showToast('Gagal rolling siswa: ' + (err.response?.data?.message || err.message), 'error');
        } finally {
            handleCloseModal(); 
        }
...
            onDelete={handleDeleteClick}
            onRollingStudents={handleOpenRollingModal}
...
      />
    <RollingStudentsModal
        isOpen={isRollingModalOpen}
        onClose={handleCloseModal} 
        teacher={selectedTeacherForRolling}
        onSave={handleSaveRolling}
    />
