...
const AdminJournalView = () => {
  ...
  const handleCloseToast = () => setToast({ message: '', type: '', id: null });
  ...
  useEffect(() => {
    const fetchJournals = async () => {
      ...
      try {
        const data = await journalApi.getJournalToday();
        ...
        setToast({ message: 'Gagal memuat jurnal.', type: 'error', id: Date.now() });
      } finally {
  ...
  const handleExport = () => {
    ...
    const filteredByMonth = journals.filter(j => {
    ...
    autoTable(pdf, {
      ...
      body: filteredByMonth.map((j, idx) => [
        idx + 1,
        j.name,
        j.judulJurnal,
        j.deskripsi,
