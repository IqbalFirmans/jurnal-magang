...
const TeacherFormModal = ({
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        password_confirmation: '',
        fotoGuruFile: null,
        fotoGuruPreviewUrl: initialData.image || null,
      });
    } else {
      setFormData({
        namaLengkap: '',
  ...
  const handleFotoGuruChange = useCallback((file, previewUrl) => {
    setFormData((prev) => ({
      ...prev,
      fotoGuruFile: file,
      fotoGuruPreviewUrl: previewUrl,
    }));
    if (localValidationErrors.fotoGuruFile) {
      setLocalValidationErrors((prev) => ({ ...prev, fotoGuruFile: undefined }));
    }
  }, [localValidationErrors]);
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      teacherFormSchema(isEditingMode).parse(formData);

      const dataToSend = new FormData();
      dataToSend.append('name', formData.namaLengkap);
      dataToSend.append('nuptk', formData.nuptk);
      ...
      if (formData.fotoGuruFile) {
        dataToSend.append('image', formData.fotoGuruFile);
      } else if (isEditingMode && !formData.fotoGuruPreviewUrl) {
  ...
  const displayErrors = {
    ...localValidationErrors,
    ...apiErrors,
    fotoGuruFile: localValidationErrors.fotoGuruFile
  };
        ...
        <div className="p-6">
          ...
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-gray-800 dark:text-white">
              {isEditingMode ? 'Edit Data Guru' : 'Tambah Guru Baru'}
            </h2>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit}>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="col-span-1">
                <FileUploadDropzone
                  label="Foto Guru"
                  onFileChange={handleFotoGuruChange}
                  currentPreviewUrl={formData.fotoGuruPreviewUrl}
                  acceptedFileTypes="image/*"
                  heightClass="min-h-[180px]"
                  hasError={!!displayErrors.fotoGuruFile}
                />
                {displayErrors.fotoGuruFile && (
                  <p className="text-red-500 text-sm mt-1">{displayErrors.fotoGuruFile}</p>
                )}
              </div>
              ...
              <div className="col-span-1 md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <InputField
                    ...
                    onChange={handleChange}
                    hasError={!!displayErrors.nuptk}
                  />
                  {displayErrors.nuptk && (
                    <p className="text-red-500 text-sm">{displayErrors.nuptk}</p>
                  )}
                </div>
